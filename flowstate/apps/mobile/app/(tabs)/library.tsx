...
import { createCollection, getCollections, getTrackers, getTrackerSummary, updateTracker } from '@flowstate/core';
...
export default function LibraryScreen() {
  ...
  const [trackers, setTrackers] = useState<LibraryTracker[]>([]);
  const [folderStack, setFolderStack] = useState<Array<{ id: string | null; name: string }>>([{ id: null, name: 'Library' }]);
  ...
  useFocusEffect(
    useCallback(() => {
      load().catch((error) => console.error('Failed to load tracker library', error));
    }, [load]),
  );
      ...
      <View style={styles.hero}>
        <View style={{ flex: 1, gap: space[4] }}>
          <AppText variant="title1" style={{ fontWeight: '800' }}>
            {folderStack[folderStack.length - 1]?.name ?? 'Library'}
          </AppText>
          <AppText variant="subheadline" color={themeTokens.textSecondary}>
            Folder-first tracker home with quick stats, archive control, and direct drill-in to detail.
          </AppText>
        </View>
      ...
      {visibleTrackers.length === 0 && filteredFolders.length === 0 ? (
        <View style={[styles.emptyState, { backgroundColor: themeTokens.surfaceElevated, borderColor: themeTokens.border }]}>
          ...
          <AppText variant="body" color={themeTokens.textSecondary}>
            Nothing matches this folder yet.
          </AppText>
        </View>
      ) : null}
